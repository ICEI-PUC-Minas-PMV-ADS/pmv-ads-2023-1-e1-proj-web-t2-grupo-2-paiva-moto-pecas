...
export function listFilterInputsOptions(database) {
  ...
  inputsFilterOptions.innerHTML += `<h2 class="titulo-pequeno">Marcas</h2>`;

  if (brands[0] !== "empty") {
    allIOptions = [...brands, ...categories].filter(
  ...
  if (brands.includes("empty")) {
    inputsFilterOptions.innerHTML = "";
    brands.length = 0;
    inputsFilterOptions.innerHTML += `<h2 class="titulo-pequeno">Categorias</h2>`;
    allIOptions = categories.filter(
      (value, index, arr) => arr.indexOf(value) === index
    );
  }
  ...
  allIOptions.map((option) => {
    ...
    inputsFilterOptions.innerHTML += `
        <div class="divFilter">
            <input id="${option
              .split(" ")
              .join("")}" type="checkbox" name="${option}" value="${option}">
            <p>${option}</p>
        </div>`;
